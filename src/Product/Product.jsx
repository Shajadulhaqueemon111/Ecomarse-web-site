...
const Product = () => {
    ...
    useEffect(() => {
        fetch(`https://dummyjson.com/products/${data.id}`)
            ...
            .then(data => setProduct(data)) // Corrected: Use 'data' directly
            .catch(error => console.error(error)); // Handle fetch errors
    }, [data.id]);
    const {id,thumbnail,price,title,image}=product || {}
    return (
        <div>
            <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 h-96">
    <a href="#">
        <img class="p-8 rounded-t-lg h-52 w-full" src={thumbnail} alt="product image" />
    </a>
    <div class="px-5 pb-5 ">
        <a href="#">
